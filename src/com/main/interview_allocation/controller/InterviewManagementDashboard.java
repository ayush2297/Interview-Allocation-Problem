...
import com.main.interview_allocation.utilities.InputUtility;
...
public class InterviewManagementDashboard {
    ...
    private InputUtility registrationService;
    ...
    public InterviewManagementDashboard() {
        registrationService = new InputUtility();
    ...
    private List<Attendee> registerAttendees() {
        ...
        return registrationService.getList(ATTENDEE_ID_FORMAT).stream().map(Attendee::new).collect(Collectors.toList());
    }

    private List<Interviewer> registerInterviewers() {
        System.out.println("Interviewer registration (Format : <name>");
        return registrationService.getList(INTERVIEWER_ID_FORMAT).stream().map(Interviewer::new).collect(Collectors.toList());
    }

    private List<InterviewRoom> registerRooms() {
        System.out.println("Room registration (Format : R<room-no>)");
        return registrationService.getList(ROOM_ID_FORMAT).stream().map(InterviewRoom::new).collect(Collectors.toList());
    }
